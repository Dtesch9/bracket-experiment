...
export function sliceTeamsFromRoundIndex(totalTeamsCount = TEAM_COUNT, round: number) {
...
export function lineCoords() {
  ...
  for (let row = 0; row < getNumberOfRounds(TEAM_COUNT); row++) {
    ...
    for (let column = 0; column < sliceTeamsFromRoundIndex(TEAM_COUNT, row).length; column++) {
